...
function pickSceneLocal(request) {
  ...
  const point = findValidScenePointLocal(
    localAspect,
    request.searchDepth,
...
function findValidScenePointLocal(localAspect, searchDepth, searchZoomFactor, searchGridSize, minimumRadius, scenePowerValue) {
  let fallback = null;
  let best = null;
  ...
  for (let attempt = 0; attempt < PICK_MAX_RETRIES; attempt += 1) {
    const point = pickScenePointLocal(
      localAspect,
      searchDepth,
      zoomFactor,
      searchGridSize,
...
function pickScenePointLocal(localAspect, searchDepth, searchZoomFactor, searchGridSize, minimumRadius, scenePowerValue) {
  ...
  for (let level = 0; level < searchDepth && radius > minimumRadius; level += 1) {
    ...
    for (let j = 0; j < rows; j += 1) {
      ...
      for (let i = 0; i < cols; i += 1) {
        ...
        const escapeIter = measureEscapeIterationsLocal(x0, y0, maxIter, scenePowerValue);
    ...
    const pickedSceneIdx = pickSceneCandidateIndexLocal(
      boundaryIndices,
      boundaryCount,
      slowOutsideIndices,
      slowOutsideWeights,
...
function pickSceneCandidateIndexLocal(boundaryIndices, boundaryCount, slowOutsideIndices, slowOutsideWeights, slowOutsideCount) {
  ...
  return pickWeightedCandidateIndexLocal(slowOutsideIndices, slowOutsideWeights, slowOutsideCount);
}

function pickWeightedCandidateIndexLocal(indices, weights, count) {
  let totalWeight = 0;
...
function measureEscapeIterationsLocal(x0, y0, maxIter, scenePowerValue) {
  let xVal = x0;
  let yVal = y0;
  let n = 0;
